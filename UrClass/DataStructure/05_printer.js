...
function queuePrinter(bufferSize, capacities, documents) {
    ...
    let counter = 0;
    let wait = [...documents]; 
    let printer = Array.from({length: bufferSize}, () => 0);
    //문서를 순회하면서 이루어질 듯 합니다.
    while (true) {
        // 1초 지남
        counter = counter +1;
        //용량 여유있음
        printer = [...printer.slice(1),0]
        if (!printerIsFull(printer, capacities, wait[0])) {
          printer[printer.length-1] = wait.shift() ?? 0;
        }
        if(wait.length === 0 && Math.max(...printer) === 0) return counter
    }
  }
  
  function printerIsFull(printer, capacity, newPrint) {
    let sum = printer.reduce((prev, curr) => prev+curr, 0)
    return capacity < sum + newPrint
  }
